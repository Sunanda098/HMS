...
class sale_order(models.Model):
    ...
    def onchange_product_ean(self):
        ...
        if lot_ids:
            ...
            batch_no = lot_ids.id
            exp_date = lot_ids.life_date
            price_unit = lot_ids.sale_price
            uom = lot_ids.sale_uom.id
        else:
        ...
        for o_line in self.order_line:
            if o_line.product_id == product:
                ...
                xorder_line.append((0,0,{
                    ...
                    'tax_id': o_line.tax_id,
                    'partner_id' : self.partner_id and self.partner_id.id,
                    'product_uom_qty' : (o_line.product_uom_qty + 1),
                    'price_unit': o_line.price_unit,
                    'batch_no' : batch_no or False,
                    'exp_date' : exp_date or False,
                    'product_uom': o_line.product_uom,
                    'state' : 'draft',
                }))
        if flag:
            xorder_line.append((0,0, {

...
class MedicamentGroupLine(models.Model):
    ...
    template_name = fields.Char(realted='template.name',string="Template")
    product_id = fields.Many2one('product.product', ondelete='restrict', string='Medicine Name',domain=[('hospital_product_type', '=', 'medicament')])
    indication = fields.Many2one('hms.diseases', ondelete='cascade',string='Disease', help='Choose a disease for this medicament from the disease list. It'\
    ...
    active_component_ids = fields.Many2many('active.comp','medica_group_comp_rel','medica_id','group_id',string='Active Component')
...
class IndimediMedicamentGroup(models.Model):
    ...
    physician_id = fields.Many2one('hms.physician', ondelete='set null', string='Doctor')
    diseases_id = fields.Many2one('hms.diseases', ondelete='set null', string='Diseases')
    medicine_list = fields.One2many('medicament.group.line', 'line_id', string='Medicament line')
...
class product_template(models.Model):
    ...
    suffix_frequency_id = fields.Many2one('medication.dosage', ondelete='cascade', string='Suffix Frequency')
    content_ids = fields.Many2many('medicament.content', 'medicament_content_rel', 'template_id', 'content_id', 'Generic')
    days = fields.Integer('Days', default=0)
...
#                                                  ('consultation_paediatric','Consultation Paediatric')], 
    ...
    base_condition = fields.Many2one("hms.diseases", ondelete='cascade', string="Base Condition")
    route = fields.Many2one('drug.route', ondelete='cascade', string='Route', help='')
    form = fields.Many2one('drug.form', ondelete='cascade', string='Form',help='Drug form, such as tablet or gel')
    common_dosage = fields.Many2one('medication.dosage', ondelete='cascade', string='Frequency', help='Drug form, such as tablet or gel')
...
#                                         ], string="Department",default='orthopedic')
    department_ids = fields.Many2many('hospital.department', 'medicament_department_rel', 'template_id', 'department_id', 'Departments')
#     department_type = fields.Selection(related="department_id.department_name",string="Department")
    ...
    service_type_ids = fields.One2many('service.type','pro_id','Service Type')

class ProductProduct(models.Model):
    _inherit = "product.product"

    med_group_ids = fields.One2many('medicament.group.line', 'product_id', 'Medicament Groups')
    
class ServiceType(models.Model):
    _name="service.type"
    
    service_type = fields.Selection([('hospital','Hospital'),('primary_consultation','Primary Consultation'),
                                     ('secondary_consultation','Secondary Consultation'),('referred','Referred')],'Service type')

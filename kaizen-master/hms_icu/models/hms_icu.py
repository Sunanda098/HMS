...
class ICUWelcome(models.Model):
    ...
    name = fields.Char("ID", default="New")
    ...
    gender =  fields.Selection(related='patient_id.sex', string='Sex')
    ...
    def create(self, vals):
        ...
        res.name = self.env['ir.sequence'].next_by_code('icu.welcome') or ''
        return res
...
class ICUChart(models.Model):
    ...
    gender =  fields.Selection(related='patient_id.sex', string='Sex', readonly=True)
    ...
    @api.multi
    def indimedi_icu_chart_two(self):
        action = self.env.ref('hms_icu.action_icu_chart2')
        result = action.read()[0]
        result['context'] = {'default_inpatient_id': self.inpatient_id.id,
            'default_patient_id': self.patient_id.id,
...
class ICUPrescription(models.Model):
    ...
    common_dosage = fields.Many2one('medication.dosage', ondelete='cascade', string='Frequency')
...
class ICUPrescriptionDiabetes(models.Model):
    ...
    def onchange_diabetes_rbs_value(self):
        if self.diabetes_rbs == "morethen351":
            self.diabetes_msg = 'Inform'
        elif self.diabetes_rbs == "lessthen80":
            self.diabetes_msg = 'Inform'
        else:
            self.diabetes_msg = ''

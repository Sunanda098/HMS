...
class PrescriptionGroupLine(models.Model):
    _name = 'prescription.group.line'
    medicament_group_id = fields.Many2one('medicament.group', 'Group Name')
    group_id = fields.Many2one('prescription.group', 'Group Id')


class PrescriptionGroup(models.Model):
    _name = 'prescription.group'
    ...
    group_line = fields.One2many('prescription.group.line', 'group_id', string='Medicament line')
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        args = args or []
        domain =[]
        if self._context.has_key('diagnosis_id') and self._context.get('diagnosis_id',False):
            diagnosis = self.env['hms.diseases'].search([('id','in',self._context.get('diagnosis_id',False)[0][2])])
...
class Patient(models.Model):
    ...
    bmi_ids = fields.One2many('patient.bmi','patient_id',string="BMI Details")
...
#     _order='sequence,date_start,ranking_order desc'
    ...
    def _get_wait_list(self):
        for app in self:
            app.waiting_ids = self.search([('physician_id','=',self.physician_id.id),('state', '=', 'waiting'), ('patient_id', '!=', app.patient_id.id), ('date', '>=', fields.Date.today()),('date', '<', (datetime.today() + relativedelta(days=1)).strftime('%Y-%m-%d'))])
...
#                 rank += 1 
    ...
    group_id = fields.Many2one('prescription.group',string='Group', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
...
#     inv_images = fields.Binary('Images')
    ...
    specialty = fields.Many2one(related="physician_id.specialty",  ondelete='set null', string='Speciality', help='Specialty Code')
...
#     last_name = fields.Char(string='Last Name', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
    ...
    waiting_ids = fields.One2many('hms.appointment', compute="_get_wait_list", string="Waiting")
...
#     diagnosis_id = fields.Many2one('hms.diseases', 'Diagnosis', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
    diagnosis_id = fields.Many2many('hms.diseases', 'rel_diagnosis_app', 'diagnosis_id','appointment_id' , string='Diagnosis', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]})
...
#     ranking_order = fields.Integer(string="Ranking", default=80)
    ...
    p_occupation = fields.Char(string="Occupation",related='patient_id.occupation')
    ...
    refer_type1 = fields.Selection(related='patient_id.refer_type1', string="Referred Type")
    ...
    special_price_receptionist = fields.Float('Special Charges', readonly=True)
...
#                     })
    ...
    def create_invoice(self):
        ...
        #     price = 0
        # if self.special_day:
        #     price -= product_id.special_day_price
...
#Block Bill To Hospital Default Functionality Needed  13 Feb
    ...
    @api.multi
    def action_open_s1_preview(self):
...
class PrescriptionLineWizard(models.Model):
    ...
    common_dosage = fields.Many2one('medication.dosage', string='Frequency', help='Drug form, such as tablet or gel')
    appointment_id = fields.Many2one('hms.appointment','Appointment')

...
class NewbornExaminationsFollow(models.Model):
    ...
    appointment_id = fields.Many2one('hms.appointment', 'Appointment', invisible=True)
    nnr = fields.Char('NNR')
...
class Patient(models.Model):
    ...
    def _compute_apppointment_ids(self):
        for patient in self:
            appointment_ids = self.env['hms.appointment'].search([('patient_id', '=', patient.id)])
            patient.appointment_ids = appointment_ids.ids
    ...
    nnr = fields.Char('NNR')
    appointment_ids = fields.One2many('hms.appointment', compute='_compute_apppointment_ids', string='Appointment', domain="[('patient_id', '=', id)]")
...
class PrescLine(models.Model):
    ...
    pae_appointment_id = fields.Many2one('hms.appointment', 'Appointment')
    minimum = fields.Float(compute="_get_min_max", string='Minimum')
    maximum = fields.Float(compute="_get_min_max", string='Maximum')
    content_id = fields.Many2one('medicament.content', string='Content')
...
class PaediatricAppoin(models.Model):
    ...
    def _get_pae_service_id(self):
        print "============",self.pae_product_id.search([('hospital_product_type', '=', "consultation_paediatric")], limit=1).id
...
class Vaccination(models.Model):
    ...
    appointment_id = fields.Many2one('hms.appointment','Appointment', help="Patient for vaccination")

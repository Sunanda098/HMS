...
class IndimediInstruction(models.Model):
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        args = args or []
        domain =[]
        if self._context.has_key('patient_id') and self._context.get('patient_id',False):
            patient = self.env['hms.patient'].search([('id','=',self._context.get('patient_id',False))])
            domain += [('gender','=',patient.sex)]
        if self._context.has_key('gender') and self._context.get('gender',False):
            domain += [('gender','=',self._context.get('gender'))]
        if self._context.has_key('instruction_group_id') and self._context.get('instruction_group_id',False):
...
class Hospitalization(models.Model):
    ...
    state = fields.Selection([
        ...
        ('reserved', 'Reserved'),('hosp','Hospitalized'),
        ('discharged', 'Discharged'),('cancel', 'Cancelled'),
        ('done', 'Done'),], string='Status', default='draft', track_visibility='always')
    patient_id = fields.Many2one('hms.patient', ondelete="restrict", string='Patient', track_visibility='always', states={'hosp': [('readonly', True)], 'discharged': [('readonly', True)], 'done': [('readonly', True)]})
    ...
    department_id = fields.Many2one(related='patient_id.department_id', ondelete="restrict", string='Department')
    ...
    ward_id = fields.Many2one('hospital.ward', ondelete="restrict", string='Ward/Room', states={'hosp': [('readonly', True)], 'discharged': [('readonly', True)], 'done': [('readonly', True)]})
    ward_package_id = fields.Many2one('product.product',ondelete="restrict",string='Package')
    bed_id = fields.Many2one ('hospital.bed', ondelete="restrict", string='Bed No.', states={'hosp': [('readonly', True)], 'discharged': [('readonly', True)], 'done': [('readonly', True)]})
...
#     treatment_at_home = fields.Text(string="Treatment to be countiued at home") 
    ...
    nc_parental = fields.Boolean('Parental nutrition')
    ...
    def create(self, values):
        ...
        hospitalization_ids = self.env['inpatient.registration'].search([('hospitalization_date','>=',datetime.strftime(datetime.strptime(res.hospitalization_date, '%Y-%m-%d %H:%M:%S').date(), '%Y-%m-%d %H:%M:%S')),('hospitalization_date','<=',datetime.strftime(datetime.strptime(res.hospitalization_date, '%Y-%m-%d %H:%M:%S').date(), '%Y-%m-%d 23:59:59')),('id','!=',res.id)])
        res.name = self.env['ir.sequence'].with_context(ir_sequence_date=str(datetime.strptime(res.hospitalization_date, '%Y-%m-%d %H:%M:%S').date())).next_by_code('inpatient.registration') or ''
            ...
            if order_id.accomodation_history_ids:
                for bed_history in order_id.accomodation_history_ids:
                    ...
                    if bed_history.bed_id.product_id.id:
                        print "<<<<<<if bed_his>>>>>>>>>>", bed_history.bed_id.product_id.id
...
#         return res
    ...
    def action_create_confirm(self):
        self.state = 'confirm'
        app_obj = self.env['hms.appointment']
        ...
        if appointments_ids:
            ...
            self.opd_note = latest_appointment.chief_complain
    ...
    @api.multi
    def done_discharge_summary(self):
...
class ResPartnerHospitalization(models.Model):
    ...
    vendors_gin = fields.Char(string='GIN')
...
class HMSSurgeryDietician(models.Model):
    ...
    diet_bmi = fields.Float(string='BMI',compute="_get_bmi_diet",digits=(16,2))

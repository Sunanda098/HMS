...
class Bed(models.Model):
    ...
    product_id = fields.Many2one('product.product', ondelete='cascade', string='Bed Product', required=1, domain=[('hospital_product_type', '=', 'bed')], context={'default_hospital_product_type': 'bed'})
...
class IndimediHospitalWard(models.Model):
    _name = 'hospital.ward'

    name = fields.Char(size=64, string='Name', required=True, help='Ward / Room code', copy=False)
    building_id = fields.Many2one('hospital.building', ondelete='restrict', string='Department')
    ...
    bed_ids = fields.One2many('hospital.bed', 'ward_id', 'Bed Line', readonly=1)

    @api.onchange('bed_typ')
    def on_change_bed(self):
        bed_ids = self.env['hospital.bed'].search([('bed_type', '=', self.bed_typ)])
        bed_lines = []
        val = {}
        for o in bed_ids:

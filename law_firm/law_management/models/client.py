...
class ClientClient(models.Model):
    ...
    client_emailP = fields.Char(
        string='Email ID(Personal)', track_visibility='onchange')
    ...
    client_nationality = fields.Char(string='Nationality')
    ...
    def create(self, values):
        if values.get('client_mobileP', '') and values.get('client_mobileW', ''):
            ...
            if mobile2.isdigit():
                ...
                raise ValidationError(
                    _('Enter Only Numerical Value in Mobile No.'))
        return super(ClientClient, self).create(values)

    @api.multi
    def write(self, values):
        if values.get('client_mobileP', '') and values.get('client_mobileW', ''):
            mobile_contact = values.get('client_mobileP', '')
            mobile_client_contact = values.get('client_mobileW', '')

...
class CaseJudgement(models.Model):
    ...
    type = fields.Selection([('criminal', 'Criminal'), ('civil', 'Civil')], string="Judgement Parent Category", required=True)
    ...
    def action_case_trail_judgement(self):
        ctx = self._context.copy() or dict()
        ctx.update({
            'judgement_id': self.id, 
            'tree_view_ref': self.env.ref('law_management.view_case_trail_evidance_tree'),
            'form_view_ref': self.env.ref('law_management.view_case_trail_evidance_form')})
        return {
            'name': _('Evidence'),
            'view_type': 'form',
            'view_mode': 'tree,form',
            'res_model': 'case.trail.evidance',
            'type': 'ir.actions.act_window',
            'domain': [('admissible_evidance', '=', True),('evidance_case', '=', self.case.id)],
            'context': ctx,
        }
...
class CaseTrailReferenceEvidence(models.Model):
    ...
    opposition_lawyer = fields.Many2one('opposition.lawyer',string='Opposition Lawyer',track_visibility='onchange')
...
class JudgementCategory(models.Model):
    ...
    type = fields.Selection(
        [('criminal', 'Criminal'), ('civil', 'Civil')], string='Parent Category')

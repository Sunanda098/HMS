...
class MatterMatter(models.Model):
    ...
    matter_seq = fields.Char(
        size=256, string='Matter Seq', readonly=True, default='New Case', copy=False)
    court_category = fields.Many2one('court.category',string="Court Category")
    caseid = fields.Char("Case ID", required=True)
    case_name = fields.Char(string='Case Name')
    type = fields.Selection(
        [('criminal', 'Criminal'), ('civil', 'Civil')],required=True, string='Parent Category')
    ...
    opp_lawyer_id = fields.Many2one('opposition.lawyer', string='Opp. Lawyer')
    ...
    file_number = fields.Char(string="File Number", track_visibility='onchange')
    file_owner = fields.Selection([('lawyer', 'Lawyer'), ('opposition', 'Opposition Lawyer'), ('client', 'Client'), ('accuse', 'Claimant / Plaintiff'), ('judge', 'Judge'), ('witness', 'Witness')], string="Current File Owner",track_visibility='onchange')
    file_lawyer = fields.Many2one('lawyer.details', string='Lawyer',track_visibility='onchange')
    file_opposition_lawyer = fields.Many2one('opposition.lawyer', string='Opposition Lawyer',track_visibility='onchange')
    ...
    _sql_constraints = [('name_uniq', 'UNIQUE(caseid)', 'Case ID Must Be Unique!')]
    ...
    @api.model
    def send_trail_secheduler_email_client(self):
    ...
    @api.model
    def send_trail_secheduler_email_lawyer(self):
        temp_obj = self.env['mail.template']
        trail_template_id = self.env['ir.model.data'].get_object_reference(
    ...
    @api.model
    def send_trail_secheduler_email_judge(self):
        temp_obj = self.env['mail.template']
        trail_template_id = self.env['ir.model.data'].get_object_reference(
    ...
    @api.model
    def send_trail_secheduler_email_opp_lawyer(self):
        temp_obj = self.env['mail.template']
        trail_template_id = self.env['ir.model.data'].get_object_reference(
    ...
    @api.model
    def send_trail_secheduler_email_accuse(self):
        temp_obj = self.env['mail.template']
        trail_template_id = self.env['ir.model.data'].get_object_reference(
